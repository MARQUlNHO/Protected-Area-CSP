...
public class CommandCommand {
    ...
    private static int executeAdd(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) {
        ...
        sender.sendMessage("§a" + type + " command added!");
        sender.sendMessage("§eArea: §6" + areaId + "  §eIndex: §6" + list.size());
        sender.sendMessage("§eDelay: §6" + delay + "t  §eUses: §6" + uses);
        sender.sendMessage("§eCommand: §6" + command);
        return 1;
    }
    ...
    private static int executeRemove(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) {
        ...
        if (index < 1 || index > list.size()) {
            sender.sendMessage("§cInvalid index. The area has " + list.size() + " " + (isEntry ? "entry" : "exit") + " command(s).");
            return 0;
        }

        AreaCommandEntry removed = list.get(index - 1);
        plugin.getAreaManager().removeAreaCommand(areaId, index - 1, isEntry);
        sender.sendMessage("§aCommand removed!");
        sender.sendMessage("§eArea: §6" + areaId + "  §eCommand: §6" + removed.getCommand());
        return 1;
    }
    ...
    private static int executeList(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) {
        ...
        sender.sendMessage("§6§l" + type + " Commands - §e" + areaId);
        sender.sendMessage("");
        if (list.isEmpty()) {
            sender.sendMessage("  §7No commands configured.");
        } else {
            for (int i = 0; i < list.size(); i++) {
                AreaCommandEntry e = list.get(i);
                sender.sendMessage("  §e[" + (i + 1) + "] §f" + e.getCommand());
                sender.sendMessage("      §7Delay: §f" + e.getDelayTicks() + "t  §7Max uses: §f" + e.getMaxUses());
            }
        }
        sender.sendMessage("");
        sender.sendMessage("§e§m                                          ");
    ...
    private static int executeUsesAdd(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) throws CommandSyntaxException {
        ...
        ProtectedArea area = plugin.getAreaManager().getAreas().get(areaId);
        if (area == null) { sender.sendMessage("§cArea not found: " + areaId); return 0; }

        List<AreaCommandEntry> list = isEntry ? area.getEntryCommands() : area.getExitCommands();
        if (index < 1 || index > list.size()) { sender.sendMessage("§cInvalid index. The area has " + list.size() + " command(s)."); return 0; }
        ...
        for (Player target : targets) {
            ...
            sender.sendMessage("§a+" + amount + " uses for §6" + target.getName() + "§a on [§6" + areaId + "/" + type + "/" + index + "§a] -> §6" + newVal);
        }
        return 1;
    }
    ...
    private static int executeUsesSet(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) throws CommandSyntaxException {
        ...
        for (Player target : targets) {
            ...
            sender.sendMessage("§aUses for §6" + target.getName() + "§a set to §6" + amount + "§a on [§6" + areaId + "/" + type + "/" + index + "§a]");
        }
        return 1;
    }

    private static int executeUsesAll(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin,
        ...
        List<AreaCommandEntry> list = isEntry ? area.getEntryCommands() : area.getExitCommands();
        if (index < 1 || index > list.size()) { sender.sendMessage("§cInvalid index."); return 0; }
        ...
        sender.sendMessage("§aUses " + action + " §6" + amount + "§a on §6" + areaId + " §a(" + type + " #" + index + ")");
        sender.sendMessage("§eOnline: §6" + online + "  §eOffline caches: §6" + offline);
        return 1;
    }
    ...
    private static int executeUsesInfo(CommandContext<CommandSourceStack> ctx, ProtectedAreaPlugin plugin, boolean isEntry) throws CommandSyntaxException {
        ...
        for (Player target : targets) {
            sender.sendMessage("§e§m                                          ");
            sender.sendMessage("§6§lUses - §e" + target.getName());
            sender.sendMessage("  §eArea: §6" + areaId + "  §eType: §6" + type);
            sender.sendMessage("");
            ...
            if (list.isEmpty()) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    sender.sendMessage("  §e[" + (i + 1) + "] §f" + entry.getCommand());

                    if (entry.getMaxUses() == -1) {
                        sender.sendMessage("      §7Uses: §funlimited");
                    } else {
                        int current = plugin.getAreaCommandManager().getUses(target, areaId, type, i);
                        int remaining = (current == -1) ? entry.getMaxUses() : current;
                        sender.sendMessage("      §7Uses remaining: §f" + remaining + " §7of §f" + entry.getMaxUses());
                    }
                }
            }
