package me.marquinho.protectedarea.api;
...
public final class ProtectedAreaServerAPI {
    ...
    // Players currently inside the area (excludes players with limit/all exceptions).
    public List<ServerPlayerEntity> getPlayersListInArea(String areaId) {
    ...
    // Deprecated: kept for source/binary compatibility with mods built against the old API.
    ...
    public int removeAreasByFolder(String folderPrefix) {
        ...
        File areasRoot = new File(plugin().getDataPath(), "Areas");
        File folder = isFlat
