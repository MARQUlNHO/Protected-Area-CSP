...
public class TeleportCommand {
    ...
    private static int executeTeleport(CommandContext<ServerCommandSource> context, ProtectedAreaInit plugin, int delayTicks, int groupSize) {
        try {
            ...
            if (targetsCollection.isEmpty()) { Messages.error(source, "protectedarea.command.teleport.no_players"); return 0; }
            ...
            if (world == null) { Messages.error(source, "protectedarea.command.teleport.world_not_found", Messages.arg("dimension", area.getDimension())); return 0; }
            List<ServerPlayerEntity> players = new ArrayList<>(targetsCollection);
            Messages.send(source, "protectedarea.command.teleport.progress", Messages.arg("count", players.size()), Messages.arg("area", areaId));
            Messages.send(source, "protectedarea.command.teleport.settings", Messages.arg("delay", delayTicks), Messages.arg("group", groupSize));
    ...
    private static void teleportPlayersInGroups(ProtectedAreaInit plugin, List<ServerPlayerEntity> players,
        ...
        handle[0] = SchedulerUtil.runTimer(() -> {
            if (index[0] >= players.size()) {
                Messages.send(source, "protectedarea.command.teleport.completed");
                handle[0].cancel();
                return;
            }
            ...
            for (ServerPlayerEntity player : group) {
                ...
                if (safePos != null) {
                    ...
                    Messages.send(source, "protectedarea.command.teleport.unsafe_location", Messages.arg("player", player.getGameProfile().getName()));
                }
            }
            index[0] = endIndex;
