...
public class ModVerificationListener {
    ...
    private void sendModCheckPacket(ServerPlayerEntity player) {
        ...
        try {
            ...
            plugin.getLogger().info("Sending mod verification to: " + player.getGameProfile().getName());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void checkAndKickPlayer(ServerPlayerEntity player) {
        ...
        if (isVerified == null || !isVerified) {
            ...
            plugin.getLogger().warn("Player " + player.getGameProfile().getName() + " kicked for not having the client mod");
        }
        cleanup(player.getUuid());
    }
