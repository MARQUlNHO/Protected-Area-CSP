...
import me.marquinho.protectedarea.client.debug.DebugState.AdvancedEntry;
import me.marquinho.protectedarea.client.debug.DebugState.ExceptionEntry;
import me.marquinho.protectedarea.client.debug.DebugState.RuleEntry;
...
public class DebugOverlayRenderer {
    ...
    private static final int COLOR_TYPE_DIM   = 0xFFC77DFF;
    ...
    private static void render(DrawContext context, net.minecraft.client.render.RenderTickCounter tickCounter) {
        ...
        String currentAreaId = state.isDimensionType()
                ? state.getCurrentAreaId()
                : (currentArea != null ? currentArea.getId() : "");
        ...
        String title = I18n.translate("debug.protectedarea.title");
    ...
    private static ProtectedArea resolveCurrentArea(MinecraftClient client, AreaTracker tracker, String debugType) {
        if (DebugState.TYPE_DIMENSION.equals(debugType)) return null;
        ...
        for (ProtectedArea area : tracker.getAreas().values()) {
            if ("flat".equals(debugType)) {
                if (area.isFlat() && area.isWithinFlatBounds(x, y, z, dimension)) return area;
            } else {
                if (!area.isFlat() && area.isInside(x, y, z, dimension)) return area;
            }
        }
        return null;
    }
    ...
    private static List<Line> buildContentLines(MinecraftClient client, DebugState state, ProtectedArea area,
        ...
        String debugType = state.getDebugType();
        boolean isFlat = state.isFlatType();
        boolean isDimension = state.isDimensionType();

        if (currentAreaId.isEmpty()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.area_none"), COLOR_NO_AREA));
        } else {
            lines.add(new Line(I18n.translate("debug.protectedarea.area_prefix") + currentAreaId, COLOR_AREA_ID));
        }
        int typeColor = isDimension ? COLOR_TYPE_DIM : (isFlat ? COLOR_TYPE_FLAT : COLOR_TYPE_CUBE);
        lines.add(new Line("Type: " + debugType, typeColor));
        ...
        if (isFlat) {
            ...
            switch (state.getCurrentPage()) {
                case DebugState.DIM_PAGE_RULES      -> buildRules(lines, state);
                case DebugState.DIM_PAGE_EXCEPTIONS -> buildExceptions(lines, state);
                case DebugState.DIM_PAGE_ADVANCED   -> buildAdvanced(lines, state);
                default                             -> buildDimensionOverview(lines, state);
            }
        } else {
            switch (state.getCurrentPage()) {
                case DebugState.PAGE_OVERVIEW   -> buildOverview(lines, client, tracker, false, area);
    ...
    private static void buildOverview(List<Line> lines, MinecraftClient client, AreaTracker tracker, boolean flatMode, ProtectedArea currentArea) {
        ...
        lines.add(new Line(I18n.translate("debug.protectedarea.overview.total_areas") + total, COLOR_VALUE));

        if (!flatMode && currentArea != null) {
            lines.add(new Line(I18n.translate("debug.protectedarea.overview.priority") + currentArea.getPriority(), COLOR_VALUE));
        }
        ...
        if (client.player != null) {
            ...
            lines.add(new Line(I18n.translate("debug.protectedarea.overview.position") + x + ", " + y + ", " + z, COLOR_COORDS));
            ...
            if (flatMode && currentArea != null) {
                ...
                String sideLabel = I18n.translate(positive
                        ? "debug.protectedarea.overview.side_positive"
                        : "debug.protectedarea.overview.side_negative");
                lines.add(new Line(I18n.translate("debug.protectedarea.overview.side") + axisName + ": " + sideLabel, positive ? COLOR_SIDE_POS : COLOR_SIDE_NEG));
                lines.add(new Line(I18n.translate("debug.protectedarea.overview.plane") + String.format("%.2f", currentArea.getFlatPlaneCoord()), COLOR_VALUE));
            }
        }
    }

    private static void buildDimensionOverview(List<Line> lines, DebugState state) {
        lines.add(new Line(I18n.translate("debug.protectedarea.overview.dimension") + state.getDimensionKey(), COLOR_VALUE));
        lines.add(new Line(I18n.translate("debug.protectedarea.overview.total_dimension_areas") + state.getTotalDimensionAreas(), COLOR_VALUE));
        lines.add(new Line(I18n.translate("debug.protectedarea.overview.priority") + state.getDimensionPriority(), COLOR_VALUE));

        String skybox = state.getDimensionSkybox();
        boolean noSkybox = skybox == null || skybox.isEmpty();
        String skyboxLabel = noSkybox ? I18n.translate("debug.protectedarea.overview.skybox_none") : skybox;
        lines.add(new Line(I18n.translate("debug.protectedarea.overview.skybox") + skyboxLabel,
                noSkybox ? COLOR_NO_AREA : COLOR_VALUE));
    }

    private static void buildRules(List<Line> lines, DebugState state) {
        List<RuleEntry> rules = state.getRules();
        if (rules.isEmpty()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.rules.no_rules"), COLOR_NO_AREA));
            return;
        }
        for (RuleEntry rule : rules) {
            if (rule.hasException()) {
                lines.add(new Line("✔ " + rule.key() + I18n.translate("debug.protectedarea.rules.exception_suffix"), COLOR_RULE_EX));
            } else {
    ...
    private static void buildExceptions(List<Line> lines, DebugState state) {
        ...
        if (exceptions.isEmpty()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.exceptions.none"), COLOR_NO_AREA));
            return;
        }
    ...
    private static void buildLimit(List<Line> lines, DebugState state) {
        if (!state.isHasLimit()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.limit.none"), COLOR_NO_AREA));
            return;
        }
        ...
        lines.add(new Line(I18n.translate("debug.protectedarea.limit.players") + current + " / " + max, full ? COLOR_LIMIT_WARN : COLOR_LIMIT_OK));

        if (state.isLimitBlocked()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.limit.status_blocked"), COLOR_LIMIT_WARN));
        } else {
            lines.add(new Line(I18n.translate("debug.protectedarea.limit.status_open"), COLOR_LIMIT_OK));
        }

        if (state.isLimitException()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.limit.exception"), COLOR_RULE_EX));
        }
    }
    ...
    private static void buildAdvanced(List<Line> lines, DebugState state) {
        ...
        if (entries.isEmpty()) {
            lines.add(new Line(I18n.translate("debug.protectedarea.advanced.none"), COLOR_NO_AREA));
            return;
        }
    ...
    private static String buildNavString(DebugState state) {
        if (state.isFlatType()) {
            return I18n.translate("debug.protectedarea.page.overview") + " (1/1) [↑ dimension]";
        }

        int page  = state.getCurrentPage();
        int total = state.getTotalPages();

        String[] keys = state.isDimensionType()
                ? new String[]{
                        "debug.protectedarea.page.overview",
                        "debug.protectedarea.page.rules",
                        "debug.protectedarea.page.exceptions",
                        "debug.protectedarea.page.advanced"
                }
                : new String[]{
                        "debug.protectedarea.page.overview",
                        "debug.protectedarea.page.rules",
                        "debug.protectedarea.page.exceptions",
                        "debug.protectedarea.page.limit",
                        "debug.protectedarea.page.advanced"
                };

        String name = (page < keys.length) ? I18n.translate(keys[page]) : "Page " + (page + 1);
        String next = state.isDimensionType() ? "[↑ cube]" : "[↑ flat]";
        return name + " (" + (page + 1) + "/" + total + ") " + next;
    }
