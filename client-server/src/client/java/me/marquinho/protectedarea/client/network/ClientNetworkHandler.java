package me.marquinho.protectedarea.client.network;
...
public class ClientNetworkHandler {
    ...
    public static void sendPlayerEnteredArea(String areaId) {
        try {
            ClientPlayNetworking.send(new ProtectedAreaPayload(createPayload("PLAYER_ENTERED_AREA", areaId)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void sendPlayerLeftArea(String areaId) {
        try {
            ClientPlayNetworking.send(new ProtectedAreaPayload(createPayload("PLAYER_LEFT_AREA", areaId)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void requestExpel(String areaId) {
        try {
            ClientPlayNetworking.send(new ProtectedAreaPayload(createPayload("REQUEST_EXPEL", areaId)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void requestReturnToArea(String areaId) {
        try {
            ClientPlayNetworking.send(new ProtectedAreaPayload(createPayload("REQUEST_RETURN", areaId)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void requestCollisionNotification(String areaId, boolean isNoEntry) {
        try {
            ...
            ClientPlayNetworking.send(new ProtectedAreaPayload(bos.toByteArray()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void requestDebugPage(int page) {
        requestDebugPage(page, "cube");
    }

    public static void requestDebugPage(int page, String scope) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            DataOutputStream out = new DataOutputStream(bos);
